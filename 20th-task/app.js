...
let sortedPopulation = countries_data.sort(
  (a, b) => b.population - a.population
);

let first10 = sortedPopulation.slice(0, 10);

console.log(first10);

let sortedArea = countries_data.sort((a, b) => b.area - a.area);
let first10Area = sortedArea.slice(0, 10);

console.log(first10Area);

let wrldPop = 0;
sortedPopulation.forEach((country) => (wrldPop += country.population));

console.log(wrldPop);

$(".population").click(() => {
    $(".text").text("10 Most populated countries")
  $(".countries").empty();
  first10.map((elem) => {
    let element = `<div> <a>${elem.name}</a> <span style: width:"100%"><span style: width: "100%"></span></span> <a>${elem.population}</a> </div>`;
    $(".countries").append(element);
  });
});

$(".area").click(() => {
    $(".text").text("10 Most spoken languages");
  $(".countries").empty();
  first10Area.map((elem) => {
    let element = `<div> <a>${elem.name}</a> <span style: width:"100%"><span style: width: ""></span></span> <a>${elem.area}</a> </div>`;
    $(".countries").append(element);
  });
});
